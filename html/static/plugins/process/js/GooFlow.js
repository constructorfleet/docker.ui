...
//Initialize区域图的Object
	...
	this.$focus="";//Current被选定的Node/Convert LinesID,If没SelectOr..工作区被Clear,And..""
	this.$cursor="default";//The style of the mouse cursor in the working area
	this.$editable=false;//Is the workspace editable
	this.$deletedItem={};//在Flowchart的EditOperation中被Delete掉的ElementsIDGather,ElementsID为KEY,ElementsType(node,line.area)为VALUE
	var headHeight=0;
	var tmp="";
	if(property.haveHead){
		...
		headHeight=24;
		//以下是当Toolbarbutton被Click时Trigger的Events自Definitions(Fake),Format为function(),Because it worksTHISOperationObject本身,不用Participation；Users can define their own weights:
		this.onBtnNewClick=null;//NewFlowchartbutton被点中
		this.onBtnOpenClick=null;//打开FlowchartbuttonDefinitions
		this.onBtnSaveClick=null;//SaveFlowchartbuttonDefinitions
		this.onFreshClick=null;//重载FlowchartbuttonDefinitions
		...
		this.$head.on("click",{inthis:this},function(e){
			...
			//Definitions顶部Operation栏buttonEvents
	...
	if(property.haveTool){
		...
		if(property.toolBtns&&property.toolBtns.length>0){
			...
			for(var i=0;i<property.toolBtns.length;++i){
				tmp+="<a href='javascript:void(0)' type='"+property.toolBtns[i]+"' id='"+this.$id+"_btn_"+property.toolBtns[i].split(" ")[0]+"' class='GooFlow_tool_btn'><b class='ico_"+property.toolBtns[i]+"'/></a>";//Add自Definitionsbutton
			}
			this.$tool.append(tmp);
		}
		//Add区域划分框Tools开关button
	...
	this.$draw=null;//画矢量Line的Containers
	...
	if(this.$editable){
	  ...
	  //为了Node而Increase的Some集体delegateTie
	  ...
	  this.$lineMove=$("<div class='GooFlow_line_move' style='display:none'></div>");//Operation折线时的Move框
	  ...
	  this.$lineOper=$("<div class='GooFlow_line_oper' style='display:none'><b class='b_l1'></b><b class='b_l2'></b><b class='b_l3'></b><b class='b_x'></b></div>");//选定线时Show的Operation框
	  ...
	  //Down thereTie当Node/线/Group Blocks的SomeOperationEvents,这些Events可直接PassthisVisitor per se
	  //当Operation某个单元（Node/线/Group Blocks）被Add时，Method of triggering，BackFALSE可StopAddEvents的发生
	  ...
	  this.onItemAdd=null;
	  //当Operation某个单元（Node/线/Group Blocks）被Delete时，Method of triggering，BackFALSE可StopDeleteEvents的发生
	  //Formatfunction(id，type)：idIt\'s the only sign of the unitID,typeIt\'s the type of unit,有"node","line","area"Three values
	  this.onItemDel=null;
	  //当Operation某个单元（Node/Group Blocks）When moved，Method of triggering，BackFALSE可StopMoveEvents的发生
	  ...
	  //当Operation某个单元（Node/线/Group Blocks）When renamed，Method of triggering，BackFALSE可Stop重命名Events的发生
	  ...
	  //当Operation某个单元（Node/线）被由Uncheck变成Select时，Method of triggering，BackFALSE可StopSelectEvents的发生
	  ...
	  //当Operation某个单元（Node/线）被由Select变成Uncheck时，Method of triggering，BackFALSE可StopUncheckEvents的发生
	  //Formatfunction(id，type)：idIt\'s the only sign of the unitID,typeIt\'s the type of unit,有"node","line"Two values,"area"不Support被Uncheck
	  ...
	  if(property.useOperStack&&this.$editable){//If要Use堆栈RecordsOperation并Provision“Undo/Redo”的Functions,Valid only in editing status
		this.$undoStack=[];
		this.$redoStack=[];
		this.$isUndo=0;
		///////////////以下是构造UndoOperation/RedoOperation的Methodology
		//To save browser memory space,undo/redoMediumOperation缓存栈,At most40步Operation;Over40Step,将Auto删掉最旧One缓存
		...
		//将External的MethodologyAdd到GooFlowobject事务Operation堆栈中,After thatundo/redoOperation中YeahConductControl，一般For rightFlowchart以外的附加InformationConductEdit的事务Undo/Redo Control；
		//ParticipationfuncFor the object to execute the method,jsonParaIt\'s the only word-oriented external methodJSONParticipation,由JSONObject带入All要传Information；
		//Hint:In order for external methods to be able to..UNDO/REDO,Need to develop these external methods,Add对该MethodologyImplementation后EffectsBack的另一个ImplementationMethodology的pushExternalOper
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		var elem;
		if(GooFlow.prototype.useSVG!=""){
			this.$draw=document.createElementNS("http://www.w3.org/2000/svg","svg");//可Create带有Assign命名空间的ElementsNodes
		...
		//TieConnect的ClickSelectand双击EditEvents
		var tmpClk=null;
	...
	initGroup:function(width,height){
		this.$group=$("<div class='GooFlow_work_group' style='width:"+width*3+"px;height:"+height*3+"px'></div>");//存放Background区域的Containers
		this.$workArea.prepend(this.$group);
		if(!this.$editable)	return;
	  //区域划分框Operation区的EventsTie
	  this.$group.on("mousedown",{inthis:this},function(e){//TieRESIZEFunctionsandMoveFunctions
		if(e.button==2)return false;
		var This=e.data.inthis;
	  ...
	  //TieModify文字AnnotationsFunctions
	  ...
	  this.$group.mouseup({inthis:this},function(e){
		...
		switch(e.target.tagName){
			...
			case "B"://Bind Colour Transforming
	...
	//每一种TypeNode及其buttonAnnotations文字
	...
	//SwitchLeftToolbarbutton,ParticipationTYPEOrganisationSwitch成哪种Type的button
	...
	//Increase一个流程Node,♪ To be heard ♪JSON,有id,name,top,left,width,height,type(NodeType)Equal Properties
	addNode:function(id,json){
		...
		if(this.$editable){
			...
			if(this.$deletedItem[id])	delete this.$deletedItem[id];//在BackDeleteOperation时,Get rid of it该Elements的DeleteRecords
		}
	},
	...
	//AccessNode/Connect/Detailed information on cluster areas
	...
	//CancelAllNode/Connect to selected status
	blurItem:function(){	
		if(this.$focus!=""){
			var jq=$("#"+this.$focus);
	...
	//选定某个Node/Convert Lines bool:TRUE决定了要TriggerSelectEvents，FALSE则Do not triggerSelectEvents，More Call within Program。
	focusItem:function(id,bool){
		var jq=$("#"+id);
		if(jq.length==0)	return;
		if(!this.blurItem())	return;//Execute first"Uncheck",IfBackFLASE,则也会Stop选定EventsGo onConduct.
		if(jq.prop("tagName")=="DIV"){
	...
	//SettingsNode的尺寸,仅Support非Start/End Node
	...
	delNode:function(id){
		...
		if(this.$editable){
			//在Back新增Operation时,IfNodesID以this.$id+"_node_"Start,则Organisation为本次Edit时新Add的Nodes,这些Nodes的Delete不用Add到$deletedItem中
			if(id.indexOf(this.$id+"_node_")<0)
				this.$deletedItem[id]="node";
		}
	},
	//SettingsFlowchart的Name
	...
	//用AJAXModalities，RemoteRead一组Data
	//Parameterspara为JSONStructure，与JQUERY中$.ajax()Methodology的Participation一样
	...
	//把画好的整个FlowchartExport到一个Variables中(It\'s actually a direct accessGooFlowobject$nodeData,$lineData,$areaDataThese threeJSONProperties)
	exportData:function(){
	...
	//只把本次EditFlowchart中作了变更(Including additions and deletions)的ElementsExport到一个Variables中,以EasyUser每次Edit载入的Flowchart后只Access变更过的Data
	...
	//变更Elements的ID,Usually used after quick saving,Returning backstage to the new elementIDUpdate到Page中;type为ElementsType(Nodes,Connect,Blocks)
...
///////////The following is the method of drawing lines:
	...
	//Calculate两个Node间要连LineIf..，Connect的StartCoordinates和EndCoordinates
	...
	//Calculate两个Node间要连折线If..，Connect的AllCoordinates
	calcPolyPoints:function(n1,n2,type,M){
		...
		var sp=[],m1=[],m2=[],ep=[];
		//If是AllowMiddler to move around,则ParametersMis a moving middle lineXCoordinates
		...
		//If是AllowMidrix to move up/ down,则ParametersMis a moving middle lineYCoordinates
		else if(type=="tb"){
			//Crude calculation2Midpoint
			m1=[SP.x,M];
			m2=[EP.x,M];
	...
	addLine:function(id,json){
		...
		var n1=null,n2=null;//AccessStart/End Node的Data
		if(json.from==json.to)	return;
		//Avoid两个Nodes间I can\'t有一条Above同向接Connect
		for(var k in this.$lineData){
			if((json.from==this.$lineData[k].from&&json.to==this.$lineData[k].to))
				return;
		}
		var n1=this.$nodeData[json.from],n2=this.$nodeData[json.to];//AccessStart/End Node的Data
		if(!n1||!n2)	return;
		var res;
	...
	//重构All连向某个Node的线的Show，ParticipationStructure为$nodeDataA unit structure for arrays
	resetLines:function(id,node){
		for(var i in this.$lineData){
		  var other=null;//AccessEnd/Start Node的Data
		  var res;
	...
	//重新SettingsConnectStyles newType= "sl":Line, "lr":Middle Moveable Lines, "tb":Middler moving up and down
	...
	delLine:function(id){
		...
		--this.$lineCount;
		if(this.$editable){
			//在Back新增Operation时,IfNodesID以this.$id+"_line_"Start,则Organisation为本次Edit时新Add的Nodes,这些Nodes的Delete不用Add到$deletedItem中
			if(id.indexOf(this.$id+"_line_")<0)
			this.$deletedItem[id]="line";
		}
		this.$lineOper.hide();
	},
	//Colour/Cancel标注一个Node或Convert Lines，Progress often used to show focus or process。
	...
	delArea:function(id){
		...
		--this.$areaCount;
		if(this.$editable){
			//在Back新增Operation时,IfNodesID以this.$id+"_area_"Start,则Organisation为本次Edit时新Add的Nodes,这些Nodes的Delete不用Add到$deletedItem中
	...
	//重构整个Flowchart设计器The width height
...
//将Such的构造FunctionsAdd至JQUERYObject
jQuery.extend({
